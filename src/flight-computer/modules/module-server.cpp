...
ServerModule::ServerModule(const ConfigData config_data, DataStream &stream)
    : Module(stream, configurables::prefix::kServerModule, "server"),
...
int ServerModule::checkShutdown() { return gfs_shutdown_flag_; }

void ServerModule::runner() {
  ServerSocket server_socket(
      MODULE_SERVER_PORT,
      1);  // Create non-blocking so it can be checked for shutdown
  ...
  while (!stop_flag_) {
    ...
    try {
      ...
      while (!stop_flag_) {
        ...
        if (request.empty()) {
          empty_request_count++;
          if (empty_request_count >
              10) {  // If there are 10 empty requests in a row, assume the
        ...
        if (request == "get-config") {
          ...
          sendLogFiles(new_sock);
                   ...
                   "shutdownServer") { /** @todo this should just be a command*/
          data("SOCKET", "SHUTDOWN");
          module_status_ = ModuleStatus::STOPPED;
          stop_flag_ = 1;
          return;
        } else if (request == "shutdownGFS") {
          data("SOCKET", "SHUTDOWN_GFS");
          module_status_ = ModuleStatus::STOPPED;
          stop_flag_ = 1;
      ...
      error("Socket Creation", e.description());
    }
...
void ServerModule::sendConfig(ServerSocket &socket) {
  ...
  telemetry_config["aprs-memo"] = config_data_.telemetry.aprs_comment;
  ...
  telemetry_config["data-packets-enabled"] =
      config_data_.telemetry.data_packets_enabled;
  telemetry_config["data-packets-frequency"] =
      config_data_.telemetry.data_packets_mode;
  telemetry_config["data-packets-mode"] =
      config_data_.telemetry.data_packets_mode;
  ...
  for (ExtensionMetadata extension : config_data_.extensions.extensions_list) {
    ...
    if (extension.interface == ExtensionMetadata::Interface::I2C) {
      extension_json["extra-args"] =
          "BS: " + std::to_string(extension.extra_args.I2C_bus) +
          " ADDR:" + extension.extra_args.I2C_device_address;
    } else if (extension.interface == ExtensionMetadata::Interface::ONEWIRE) {
      extension_json["extra-args"] = "ID: " + extension.extra_args.one_wire_id;
    } else {
  ...
  json config = {{"general", general_config},
...
void ServerModule::sendStatus(ServerSocket &socket) {
  ...
  std::string system_time =
      (gmt->tm_hour < 10 ? "0" + std::to_string(gmt->tm_hour)
                         : std::to_string(gmt->tm_hour)) +
      ":" + std::to_string(gmt->tm_min) + ":" + std::to_string(gmt->tm_sec);
  std::string system_date = std::to_string(gmt->tm_mday) + "-" +
                            std::to_string(gmt->tm_mon + 1) + "-" +
                            std::to_string(gmt->tm_year + 1900);

  std::string system_data_source = config_data_.extensions.system_data_name;
  std::string system_uptime_hours =
      data_stream_.getData(system_data_source, "uptime_hours");
  ...
  std::string mem_usage =
      data_stream_.getData(system_data_source, "ram_used_prcnt");
  std::string storage_usage =
      data_stream_.getData(system_data_source, "disk_used_prcnt");

  std::string battery_voltage =
      data_stream_.getData(config_data_.extensions.battery_data_name, "BAT_V");

  json system_status = {{"system-time-utc", system_time},
  ...
  std::string gfs_uptime_str =
      ...
      ":" +
      ((gfs_uptime % 3600) / 60 < 10 ? "0" + gfs_uptime_minutes
                                     : gfs_uptime_minutes) +
      ":" +
      (gfs_uptime % 60 < 10 ? "0" + gfs_uptime_seconds : gfs_uptime_seconds);

  std::unordered_map<std::string, ModuleStatus> module_statuses =
      data_stream_.getModuleStatuses();
  json gfs_status = {
      {"gfs-uptime", gfs_uptime_str},
      {"health-status", "N-I"},
      {"reported-errors", data_stream_.getNumCurrentErrors()},
      {"current-flight-proc",
       kFlightProcedureTypeToString.at(
           data_stream_.getCurrentFlightProcedure().type)},
      {"flight-phase", "N-I"},
      {"modules-status",
       {{"configuration",
         module_status_to_string_map.at(module_statuses["configuration"])},
        {"data", module_status_to_string_map.at(module_statuses["data"])},
        {"extensions",
         module_status_to_string_map.at(module_statuses["extensions"])},
        {"telemetry",
         module_status_to_string_map.at(module_statuses["telemetry"])},
        {"server", module_status_to_string_map.at(module_statuses["server"])},
        {"console",
         module_status_to_string_map.at(module_statuses["console"])}}}};

  json statuses = {{"system", system_status}, {"gfs", gfs_status}};
...
void ServerModule::sendGfsData(ServerSocket &socket) {
  ...
  json data_log_status_json = {
      {"enabled", "N-I"}, {"file-name", "N-I"}, {"file-size-mb", "N-I"}};
  ...
  json environmental_data_json = {
      {"pressure",
       {{"source1", {{"unit", "N-I"}, {"value", "N-I"}}},
        {"source2", {{"unit", "N-I"}, {"value", "N-I"}}}}},
      {"temperature",
       {{"source1", {{"unit", "N-I"}, {"value", "N-I"}}},
        {"source2", {{"unit", "N-I"}, {"value", "N-I"}}}}},
      {"humidity",
  ...
  // Calculated Data
  json calculated_data_json = {{"distance-traveled", "N-I"},
                               {"overall-heading", "N-I"},
                               {"time-since-launch", "N-I"}};

  json gfs_data = {{"data-stream", data_stream_stats_json},
...
void ServerModule::sendTelemetryData(ServerSocket &socket) {
  ...
  while (!queue.empty()) {
    ...
    tx_queue_json[std::to_string(j++)] = {
        {"file", tx.wav_location}, {"type", tx.type}, {"duration", tx.length}};
  }
  ...
  for (auto const &tx : log) {
    ...
    tx_log[std::to_string(i++)] = {{"ID", tx.tx_num},
                                   {"type", type},
                                   {"message", tx.message},
                                   {"length", tx.length}};
  }
  ...
  json telemetry_data = {{"tx-log-info", tx_log_info}, {"tx-log", tx_log}};
  try {
    socket << telemetry_data.dump();  // Send the data
  } catch (SocketException &e) {
    error("Telemetry", e.description());
  }
}
...
void ServerModule::sendDataFrame(ServerSocket &socket) {
  ...
  for (auto const &[key, packet] : snapshot) {
    data_frame[std::to_string(i++)] = {{"source", packet.source},
                                       {"unit", packet.unit},
                                       {"value", packet.value}};
  }
...
void ServerModule::sendErrorFrame(ServerSocket &socket) {
  ...
  for (auto const &[key, error] : error_snapshot) {
    errors[std::to_string(k++)] = {{"source", error.source},
...
void ServerModule::sendLogFiles(ServerSocket &socket) {
  ...
  json log_files = {{"data", data_log_files}, {"error", error_log_files}};

  try {
    socket << log_files.dump();  // Send the data
  } catch (SocketException &e) {
    error("Log Files", e.description());
  }
