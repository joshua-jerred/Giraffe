...
class TSL2591 : public PeriodicProcessor {
public:
  static constexpr uint8_t DEVICE_I2C_ADDRESS = 0x42;

  TSL2591(uint64_t module_process_interval_ms)
    ...
    // i2c_set_slave_mode(I2C_PORT, false, DEVICE_I2C_ADDRESS);
...
private:
  ...
  //   keystone::UBX::calculateChecksum(ubx_write_buffer_,
  //   ubx_write_buffer_.ck_a,
  ...
  //   uint8_t buffer[2];
  //   i2c_write_blocking(I2C_PORT, I2C_ADDRESS, &kStreamSizeMsbRegister, 1,
  //   true); i2c_read_blocking(I2C_PORT, I2C_ADDRESS, buffer, 2, false);
  ...
  //   if (bytes_available < 6) { // Not enough bytes for a message with its
  //   header
  ...
  //     i2c_read_blocking(I2C_PORT, I2C_ADDRESS, payload.data(),
  //     payload.size(),
  //                       false);

  //     int result =
  //         keystone::UBX::processPayloadAndChecksum(ubx_read_buffer_,
  //         payload);
  //     if (result < 0) {
