...
void ncurs::Window::winRefresh() {
  // box(p_window_, 0, 0);
  //  mvwprintw(p_window_, 0, 1, "%s", title_.c_str());
  wrefresh(p_window_);
}

void ncurs::Window::winClear() {
  wclear(p_window_);
  wattroff(p_window_, A_REVERSE);
  box(p_window_, 0, 0);
}

void ncurs::Window::winHide() {
  wclear(p_window_);
  wrefresh(p_window_);
}
...
void ncurs::Environment::displayMenu() {
  ...
  int i = 1;
  current_menu_num_options_ = pages_.getMenuOptions().size();

  for (MenuOption option : pages_.getMenuOptions()) {
    if (i - 1 == current_menu_hover_ && focus_ == Focus::MENU) {
      ...
      mvwprintw(menu_window_.p_window_, i++, 1, "%s", option.c_str());
      continue;
    } else {
      wattroff(menu_window_.p_window_, A_REVERSE);
    }
...
void ncurs::Environment::displayData() {
  ...
  const std::vector<std::string> &page = pages_.getPage(current_page_option_);

  int i = current_scroll_pos_;
  for (std::string line : page) {
    if (i >= K_MAX_NUM_PAGE_LINES) {
...
int ncurs::Environment::getMaxScrollPos() {
  int lines_on_page = pages_.getPage(current_page_option_).size();
