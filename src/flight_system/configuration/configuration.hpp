...
private:
  ...
  cfg::gEnum::ProcedureType starting_procedure_ =
      cfg::gEnum::ProcedureType::OTHER;
...
private:
  ...
  cfg::gEnum::TimestampDetail timestamp_detail_ =
      cfg::gEnum::TimestampDetail::HHMMSS;
  cfg::gEnum::TimestampTimezone timestamp_timezone_ =
      cfg::gEnum::TimestampTimezone::SYSTEM;
  int log_interval_ms_ = 5000;
  int max_data_log_file_size_mb_ = 10;
  int max_data_archive_size_mb_ = 100;
  cfg::gEnum::ArchiveMethod archive_method_ =
      cfg::gEnum::ArchiveMethod::PLAIN_TEXT;
...
private:
  ...
  cfg::gEnum::InfluxdbRetentionPolicy retention_policy_ =
      cfg::gEnum::InfluxdbRetentionPolicy::INF;
...
private:
  ...
  cfg::gEnum::ArchiveMethod error_archive_method_ =
      cfg::gEnum::ArchiveMethod::PLAIN_TEXT;
  cfg::gEnum::LogLevel log_level_ = cfg::gEnum::LogLevel::INFO;
  cfg::gEnum::ErrorLogStrategy error_log_strategy_ =
...
private:
  ...
  cfg::gEnum::AprsSymbolTable symbol_table_ =
      cfg::gEnum::AprsSymbolTable::PRIMARY;
  std::string symbol_ = "/";
...
public:
  Configuration(data::Streams &streams)
      :
        ...
        general(streams), data_module_data(streams),
        data_module_influxdb(streams), data_module_log(streams),
        console_module(streams), server_module(streams), system_module(streams),
        telemetry(streams), telemetry_aprs(streams), telemetry_sstv(streams),
        telemetry_data_packets(streams), extensions(streams),
        extension_module(streams), hardware_interface(streams),
        streams_(streams) {
  }
