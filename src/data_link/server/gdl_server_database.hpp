...
public:
  ...
  void createTables() {
    ...
    db_.exec("CREATE TABLE IF NOT EXISTS received_message ("
  ...
  void insertReceivedMessage(const Message &msg) {
    if (msg.getType() == Message::Type::BROADCAST ||
        msg.getType() == Message::Type::EXCHANGE) {
      SQLite::Statement query(db_,
                              "INSERT INTO received_message (id_string, type, "
                              "message) VALUES (?, ?, ?);");
      query.bind(1, msg.getIdentifierString());
      query.bind(2, msg.getData());
  ...
  std::vector<Message> getLatestReceivedMessages() {
    ...
    SQLite::Statement query(db_, "SELECT id_string, type, message FROM "
                                 "received_message ORDER BY id DESC LIMIT 10;");
    while (query.executeStep()) {
      Message msg;
      msg.setIdentifierFromHex(query.getColumn(0).getString());
      msg.setData(query.getColumn(2).getString());
      msg.setType(query.getColumn(1).getString() == "BROADCAST"
