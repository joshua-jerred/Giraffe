...
TEST(NetworkLayer, networkLayerTransmit) {
  ...
  EXPECT_TRUE(network_layer.txMessage(message)) << "Failed to transmit message";
  if (physical_layer.getState() != PhysicalLayer::State::IDLE) {
    BoosterSeat::threadSleep(1);
    EXPECT_EQ(physical_layer.getState(), PhysicalLayer::State::TRANSMITTING)
        << "Physical layer did not enter TRANSMITTING state but is not at idle";
  }
